// ==UserScript==
// @name         eTicketBot PRO (Async-Turbo)
// @namespace    http://tampermonkey.net/
// @version      1.0.1.6
// @description  Automatic Ticket Booking Script with Async Polling
...
(function() {
    ...
    // New: Polling helper to wait for a seat class change
    ...
    async function startBooking() {
        ...
        const options = Array.from(selectElement.options)
            .map(o => ({ value: o.value, text: o.textContent, count: parseInt(o.textContent.match(/(\d+)/)?.[0] || 0) }))
    ...
    async function performSeatSelection() {
        ...
        console.log("Starting Seat Selection Polling...");
        ...
        while (selectedCount < maxSeats) {
            ...
            if (available.length === 0) {
                console.log("No more available seats to try.");
                break;
            }
            ...
            // WAIT HERE until the button actually changes color/class
            ...
            if (result === "SUCCESS") {
                ...
                console.log(`>> ✗ FAILED: ${title} was grabbed by someone else.`);
            } else {
                console.log(`>> ? SKIP: ${title} (${result})`);
            }

            await sleep(100); // Small gap before next attempt
        }
